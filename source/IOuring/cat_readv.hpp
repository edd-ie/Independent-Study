...
namespace readv_impl
{
    ...
    ssize_t read_and_print_file(char *const file_name, bool quiet = false)
    {
        ...
        int blocks = static_cast<int>(file_sz) / CHUNK_SIZE;
        ...
        {
            off_t bytes_to_read = std::min<off_t>(bytes_remaining, CHUNK_SIZE);
    ...
    int readV_cat(const int argc, char *const argv[])
    {
        bool quiet = false;
        int opt;
